...
function down_count_get_data()
{
	...
	if (!isset($data))
	{
		...
		if (!file_exists( $fn )) // файла нет, массив пустой
			$data = array();
		else
...
function down_count_init($args = array())
{
	...
	if (mso_segment(1) == $options['prefix'] and mso_segment(2))
	{
		...
		{
			$data = read_file($fn);
			if ($data === false) //значит, не получилось прочитать существующий файл и его запись повредит данные
			{
				header("Location: " . getinfo('siteurl') . $options['prefix'] . mso_segment(2) );
				exit; // можем попробовать обновить страницу, чтобы ещё раз сделать всё то же самое.
			}

			$data = unserialize( $data ); // получим из файла
		...
		write_file($fn, serialize($data) ); // сохраняем в файл
