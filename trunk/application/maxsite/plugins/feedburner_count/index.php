...
function feedburner_count_admin_page($args = array())
{
	...
	mso_hook_add_dinamic( 'mso_admin_header', 'return $args . "' . t('Настройка FeedBurner Count от samborsky.com', 'plugins') . '"; ' );
	mso_hook_add_dinamic( 'admin_title', ' return "' . t('Настройка FeedBurner Count от samborsky.com', 'plugins') . ' - " . $args; ' );
...
function feedburner_count_update()
{
	...
	if( feedburner_count_need_update($options['last_update'],$options['update_interval']*60) )
	{
		...
		# старый адрес, теперь нужно братьс  гугла
		#if( $content = get_download('http://api.feedburner.com/awareness/1.0/GetFeedData?uri=' . $options['feed_name']) )
		...
		else
		{
			...
			$options['count'] = t('Ошибка!', 'plugins');
		}
