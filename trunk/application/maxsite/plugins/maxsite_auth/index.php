...
{
	echo '<span>Если у вас сайт на MaxSite CMS, то вы можете <a href="' . getinfo('siteurl') . 'maxsite-auth-form">авторизоваться с его помощью</a>.</span><br>';
...
function maxsite_auth_mso_options() 
{
	...
	mso_admin_plugin_options('plugin_maxsite_auth', 'plugins', 
		...
		'Настройки плагина Maxsite Auth', // титул
		'С помощью этого плагина вы можете осуществлять авторизацию на других сайтах с помощью своего. Достаточно лишь предварительно задать адрес email и пароль, которые будут передаваться на исходный сайт, где вы автоматически будете зарегистрированы или авторизованы как комюзер (комментатор).'   // инфо
	);
}
	...
	elseif (mso_segment(1) == 'maxsite-auth-reply')
	{
		...
		if ( $post = mso_check_post(array('data')) )
		{
			...
			*/
			...
			// имея email и пароль нужно проверить, чтобы такие были в базе
			...
			if ($query->num_rows()) // есть такой комюзер
			{
				...
				if ($comuser_info['comusers_password'] == mso_md5($pass))
				{
					...
					mso_add_to_cookie($name_cookies, $value, $expire, $data_redirect); // в куку для всего сайта
