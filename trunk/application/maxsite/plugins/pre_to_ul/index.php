...
function pre_to_ul_autoload($args = array())
{
	mso_hook_add( 'content', 'pre_to_ul_custom'); # хук на вывод контента
}

function pre_to_ul_callback($matches) 
{
	global $flag_code;
...
function pre_to_ul_custom($text) 
{
	global $flag_code;
	
	$flag_code = false;
	$pattern = '|<pre>(.*?)</pre>|si';
	$text = preg_replace_callback($pattern, 'pre_to_ul_callback', $text);
