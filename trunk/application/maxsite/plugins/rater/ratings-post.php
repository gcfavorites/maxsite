<?php if (!defined('BASEPATH')) exit('No direct script access allowed'); 
	...
	if ( $post = mso_check_post(array('rating', 'slug')) )
	{
		...
		{
			echo '<span>Вы уже голосовали!</span>';
			return; 
		}		
		...
		if ($rating) // есть присланная оценка
		{
			...
			$CI = & get_instance();
			$CI->db->select('page_rating, page_rating_count');
			...
			if ($query->num_rows() > 0)
			{
				...
				echo '<span>Ваша оценка:</span> ' . $rating . '<br /><span>Средняя оценка</span>: ' . $sredn . ' из ' . $page_rating_count . ' проголосовавших'; 
			}
		}
	}
?>