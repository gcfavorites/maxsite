 ...
 */
	...
	if ( $post = mso_check_post(array('f_session_id', 'f_submit_chek', 'f_xml_file')) )
	{
		...
		if (!isset($post['f_yes']))
		{
			echo '<div class="error">Угу, зщас... У тебя сайт накроется, кто отвечать будет?! В ЛЕС!!!</div>';
		}
		else
		{
			...
			if (file_exists($fn))
			{
				...
				if ($all)
				{
					...
					*/
					...
					$out = '<h2>Файл: ' . $f_xml_file . '</h2>';
					$out .= '<ul>';
						$out .= '<li><b>Сайт:</b> ' . $all->channel['title'];
						$out .= '<br /><b>Ссылка:</b> ' . $all->channel['link'];					
						$out .= '<br /><b>Все рубрики:</b> ' . $categorys_out;					
						// $out .= '<br /><b>Все метки:</b> ' . $tags_out;					
						$out .= '<br /><b>Всего записей:</b> ' . count($all->items);					
						$out .= '<br /><br /></li>';
					$out .= '</ul><br /><hr /><br />';
					...
					foreach($all->items as $item)
					{
						...
						if ($status == 'publish' or $status == 'static' or $status == 'draft')
						{
							...
							$out .= '<b>Статус:</b> ' . $status;
							$out .= '<br /><b>Тип страницы:</b> ' . $page_type_id;
							$out .= '<br /><b>Рубрики:</b> ' . $category;
							$out .= '<br /><b>Метки:</b> ' . $tag;
							$out .= '<br /><b>Дата:</b> ' . $post_date ;
							$out .= '<br /><b>Комментарии:</b> ' . $item['wp']['comment_status'];
							...
							if ($slug != $slug_new)
							{
								$out .= '<br /><b>Новый slug:</b> <span style="color: red"> ' . $slug_new . '</span>';
								$slug_rename++;
							}
					...
					echo $out . '<div class="update">Готово! Проверка выполнена!
								<br />Предположительно запросов к БД будет: ' . $sql_count 
								. '<br />Измененных slug (url): ' . $slug_rename 
								. '<br />Всего записей: ' . $pages_count 
								. '<br />Всего комментариев: ' . $comments_count 
								. '</div>';
				}
				else
				{
					echo '<div class="error">Что за ерунду ты мне подсовываешь? Файл-то пустой!</div>';
				}
			}
			else // нет файла
			{
				echo '<div class="error">Файл <b>' . f_xml_file. '</b> не найден! Загрузите его в каталог /uploads/ Можно через Загрузку.</div>';
			}
		}
	}
	...
	if ( $post = mso_check_post(array('f_session_id', 'f_submit_go', 'f_xml_file')) )
	{
		...
		if (!isset($post['f_yes']))
		{
			echo '<div class="error">Угу, сщас... У тебя сайт накроется, кто отвечать будет?! В ЛЕС!!!</div>';
		}
		else
		{
			$f_xml_file = $post['f_xml_file'];
			...
			if (file_exists($fn))
			{
				...
				if ($all)
				{
					...
					*/
					...
					$out .= '<h2>Добавленные страницы</h2>';
					...
					echo $out . '<br /><div class="update">Готово! Конвертирование выполнено!</div>';
...
<?php
	...
	echo '<input type="checkbox" name="f_yes" nochecked /> Я понял и согласен взять на себя всю ответственность за использование данного конвертера! Дамп также сделал и умею с ним работать<br />';
	
	echo '<br />Выберите файл: <select style="width: 300px" name="f_xml_file">' . $option_files . '</select>';
	
	echo '<br /><input type="submit" name="f_submit_chek" value=" Проверить файл " style="margin: 25px 0 5px 0;" />';
	echo '<input type="submit" name="f_submit_go" value=" Запустить конвертацию " style="margin: 25px 0 5px 0;" />';
